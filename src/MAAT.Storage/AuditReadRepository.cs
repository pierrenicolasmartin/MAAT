...
public sealed class AuditReadRepository
{
    ...
    public int CountIdentityLocations(long runId, string identity)
    {
        ...
        cmd.CommandText = """
            ...
            WHERE f.run_id = $run AND a.identity = $idn;
            """;
        cmd.Parameters.AddWithValue("$run", runId);
    ...
    /// <summary>
    /// Emplacements où une identité possède une ACE directe (son nom est le trustee),
    /// avec le détail du droit. Paginé (dossiers d'abord, par chemin) pour rester
    /// réactif sur les identités très répandues.
    /// </summary>
    public IReadOnlyList<IdentityLocationRow> GetIdentityLocations(long runId, string identity, int offset, int limit)
    {
        ...
        cmd.CommandText = """
            ...
            FROM ace a JOIN fs_item f ON a.item_id = f.id
            WHERE f.run_id = $run AND a.identity = $idn
            ORDER BY f.is_file, f.full_path COLLATE NOCASE, a.ace_type DESC
            LIMIT $limit OFFSET $offset;
            """;
        cmd.Parameters.AddWithValue("$run", runId);
        cmd.Parameters.AddWithValue("$idn", identity);
        ...
        var list = new List<IdentityLocationRow>();
        using var r = cmd.ExecuteReader();
        while (r.Read())
        {
            list.Add(new IdentityLocationRow(
                r.GetString(0), r.GetString(1), r.GetInt32(2) != 0,
                r.GetInt32(3), r.GetString(4), r.GetString(5), r.GetInt32(6) != 0));
        }
        return list;
    }
