...
public sealed class CsvExporter
{
    ...
    /// </summary>
    public void ExportToFile(string path, IEnumerable<AuditItem> items, AuditParameters p, string rootPath, int maxLevels)
    {
        using var writer = new StreamWriter(path, append: false, new UTF8Encoding(encoderShouldEmitUTF8Identifier: true));
        Export(writer, items, p, rootPath, maxLevels);
    }

    /// <summary>Écrit le CSV dans un <see cref="TextWriter"/> quelconque, en streaming.</summary>
    public void Export(TextWriter writer, IEnumerable<AuditItem> items, AuditParameters p, string rootPath, int maxLevels)
    {
