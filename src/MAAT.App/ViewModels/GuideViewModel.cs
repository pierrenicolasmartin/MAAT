...
public sealed class GuideViewModel : ObservableObject
{
    ...
    private static List<GuideTopic> BuildFr() => new()
    {
        ...
        Topic(3, "RÉSULTATS", "Explorer les résultats",
            ...
            Sec("Repères en haut",
                "Une barre de contexte rappelle le chemin audité et la date ; un bandeau de synthèse résume le nombre de dossiers (et de fichiers), la volumétrie, les entrées de droits et les identités."),
            Sec("Vue Dossiers",
        ...
        Topic(11, "CAS PARTICULIERS", "DFS, jonctions, accès refusés…",
            ...
            Sec("Accès refusés",
                "Un élément dont les droits sont illisibles reste dans l'arbre, marqué d'une pastille ambre. Un dossier dont le contenu ne peut être listé est journalisé, l'audit continue, et sa volumétrie est marquée ≈ (partielle). Le rapport de fin d'audit les récapitule tous."),
    ...
    private static List<GuideTopic> BuildEn() => new()
    {
        ...
        Topic(3, "RESULTS", "Explore the results",
            ...
            Sec("Cues at the top",
                "A context bar recalls the audited path and date; a summary band sums up the number of folders (and files), the volumetry, the permission entries and the identities."),
            Sec("Folders view",
            ...
            Sec("Special states",
                "An amber dot flags an item audited incompletely (unreadable permissions, content that could not be listed, loop avoided, maximum depth reached). The DFS and LINK tags mark DFS links and junctions / symbolic links. Select the item: a panel explains its state above the permissions table."),
        ...
        Topic(11, "SPECIAL CASES", "DFS, junctions, access denied…",
            ...
            Sec("Access denied",
                "An item whose permissions cannot be read stays in the tree, marked with an amber dot. A folder whose content cannot be listed is logged, the audit continues, and its volumetry is marked ≈ (partial). The end-of-audit report lists them all."),
